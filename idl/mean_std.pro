function mean_std,xx,wei=wei,nan=nan,STD=std
...
if keyword_set( nan ) then begin
    ...
    if ngood lt n then  begin   ; some good, use recursive call
        mean=MEAN_STD( xx[igood],wei=wei,STD=std)
        return,mean
    endif
...
if n gt 1 then begin
    if not keyword_set(wei) then mean=total(xx)/float(n) else begin
        if n_elements(wei) ne n then message,'wei must be same size as xx'
        mean = total(wei*xx)/total(wei)
    endelse
...
if arg_present(std) then begin	; do standard deviation
    if n ge 2 then begin
      if not keyword_set(wei) then var=total((xx-mean)^2) /(n-1.) $ ; variance
           else var=(total((wei*(xx-mean))^2) / total(wei^2))*(float(n)/(n-1.))
