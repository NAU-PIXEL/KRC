...
,clip=clip,fmt=fmt,evod=evod,xin=xin,csize=csize,cthick=cthick,oplot=oplot,clr=clr
...
;    j=where (finite(yy),ngood)
    ...
    if nbad eq nd then sran='all NaN' else begin
      ...
      if ymax ne ymin then begin ; have changing data
          ...
          endif else if psy eq 0 then OPLOT,XAX,y+ky,line=dine,color=clr $ ; data
                                 else OPLOT,XAX,y+ky,psym=psy,color=clr
      endif
