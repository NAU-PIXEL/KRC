...
;  nnn: from a case statement. Numeric case values should begin in column 3+ 
;
...
; ;[ = This kon is not normally directly used
;
; For ;+ lines, fuller comments can be made by usig the keys
...
;
; Case statements which uses number should be indented so that : is >7 spaces in
; 
; Processing sequence
...
; 2004nov11 getenv('IDLTOP') yields null string
...
nall=n_elements(all)
if nall eq 0 then all=0 
if nall le 1 then begin
    ...
    bit16=ishft(all,-4) mod 2 eq 1 ; +16 = Use 2nd comment line if there
    sall=strtrim(all,2)         ; string copy for comparison between calls
endif else begin 
    bit1=0 & bit2=0 & bit4=0 & bit16=0
    bit8=1B
    sall=prior[1]               ; avoid doing construction again
endelse
;;print,'bits',bit1,bit2,bit4,bit8,bit16
...
if bit16 or key eq 'n' or key eq 'N' then begin ; look for 2nd line
; will use next line, for coding simplicity, let all the tests execute and fail
    ...
    if i1 ge 0 and i1 lt 3 then begin ; if it is a comment only
        ...
        if not bit16 then print,'Missing n line; buf=',buf
    endelse
endif

if bit16 and key eq'-' then begin ; use this comment
...
if doget and i6 gt 0 then begin     ; was a GETP__ system call, extract its description
    ...
    out=' GET '+desc+': '+var
    if bit16 then begin         ; append description
        i1=strpos(buf,';',i10+1)
...
if not bit8 then begin  ; print all kon once
    if bit2 and bit4 then begin ; in LaTeX format
        for k=0,k2 do begin
            ...
            q=strmid(hold[k],0, ii[k])+space[i]+' '+text ; align output
  ...
  for k=0,k2 do begin           ; each line in guide
    if strpos(hold[k],'kons=') gt 0 then begin ; a kons line
        ...
        if nk gt 0 then for j=0,nk-1 do begin ; for each kon in list
            ...
            q=q+space[i]+' '+text & q=q[0] ; align output
; changes required for LaTeX processing
            i=strpos(q,' &')
            if i ge 0 then q=strmid(q,0,i+1)+'\&'+strmid(q,i+2)
            i=strpos(q,'<')
