...
C 2015dec24 HK First time-double no layer less than 4. FLAY now is the first 
C          physical layer. Minor:  print Changes.
C 2016feb06:mar15 HK Incorporate constant geothermal heatflow. 
...
C Compute Tg values here as may need 1,1 or 2 times
...
C
      ...
      FBI(N1) = QQ              ! " " at each layer. Temporary use of  FBI  
      ...
      DO J=N1-1,2,-1            ! LAYER LOOP 2: minimum here or below
        ...
        IF (IDB4.GE.2) WRITE(IOSP,'(i3,2f12.3)') J,SCONVG(J),FBI(J)
      ENDDO
...
C
     ...
     &    .AND. FBI(J).GT. 2.D0*QA*CONVF) THEN ! double time step for this layer
          DTIMI=2.*DTIMI
          ...
          QA=QA*2.              ! update forecast factor
        ENDIF
