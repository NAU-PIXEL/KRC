...
; 2009jun23 HK Allow for repeated values in xx; avoid divide by zero slope
;_Desc
; If repeated values, then output can depend upon LOCATE sequence, which 
; might not pick the first occurance
;_End
...
endelse
del=xx(loc+1)-xx(loc)           ; slope within interval
if del ne 0 then f=(xin-xx(loc))/del else f=0	; interpolate or extrapolate
return,loc+f
end
