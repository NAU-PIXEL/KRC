...
; yyy   in. array(n,numCurves)  Data to plot
; txt   in_ strarr(n)  Labels for curves. Default is 0-based count
...
if n_elements(xx) lt nx then xx=indgen(nx)
...
if not dop then plot,xx,xx,xran=xran,yran=yrn,/nodata $
...
for k=0,ny-1 do begin
    ...
    if doc then begin ; color
        ...
        if ksym ge 0 then oplot,xx,yy,line=lin,color=clr ; plot data line
        if ksya ne 0 then  oplot,xx,yy,psym=ksya,color=clr ; add symbol
        if dog then CURVEGUIDE,k2,gtex,lin,locc=loc2,color=clr,ksym=ksya 
        if dol then LABEL_CURVE, gtex,xx,yy,loc2[0],off=loc2[1],size=loc2[2] $
                       ,thick=loc2[3],over=loc2[4],color=clr
    endif else begin ; monochrome
        lin=((bw[k2 mod nlin])>0) mod 6 ; ensure valid line
        oplot,xx,yy,line=lin
        if ksya ne 0 then  oplot,xx,yy,psym=ksya ; add symbol
