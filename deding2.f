...
C_Titl  DEDING2 delta-eddington solution for single homogeneous layer
	IMPLICIT REAL*8 (A-H,O-Z) ! Double precision for internal variables only
C_Arg
	REAL OMEGA	![i] dust single scattering albedo
	REAL G0 	![i] dust asymmetry parameter
	REAL ASUR	![i] surface albedo
	REAL COSI	![i] cosine of incidence angle
	REAL TAU	![i] dust vertical opacity
	REAL BOND	![o] planetary (atm plus surface system) albedo
	REAL COLL	![o] Direct beam at bottom = collimated + aureole
	REAL RI(2,2)	![o] diffuse irradiances: 
...
C1	INTEGER INDX(2)		! used by ludcmp
	DATA PI/3.141592653589793D0/	! pi
	DATA ZERO,one,two,three,four/0.D0,1.D0,2.D0,3.D0,4.D0/
	DATA VLARGE,EXPMAX /1.0D200,460.51D0/ ! large real number & max exponent
	DATA GMAX /0.999D0/	! maximum magnitude of G
	DATA EPSILON /1.D-8/	! limit for 1-omega
