...
C
	IQ = IQIN
        ...
        GO TO (800,800,800,230,800,800,480,800,900,99), IQ ! print page title
C
...
146     FORMAT('0    LP1    LP2    LP3    LP4    LP5'
 ...
 143    FORMAT (/1X,A/(1X,10F7.2))
	...
	GOTO 800 ! Print page title, then come back to  150
C
...
160     FORMAT (' Conductiv.=',1PE10.3,'  Dens*Cp=',E10.3,'  Diffu.='
     &,E10.3,'  Scale=',E10.3)
        IF (IC.GT.2 .AND. IC.LT.N1-1) then
          ...
          Q6=X(IC-1)+TLAY(IC-1)/2. ! depth to top of 2nd material
          WRITE(IOSP,162),IC,Q6,Q4
 162      FORMAT(' Beginning at layer ',I3,'  At ',F8.4
     &,' m.   Inertia=',F8.1)
          Q4=COND2/Q2           ! diffusivity
          Q6=SQRT(Q4*PERIOD*86400./PI) ! Diurnal scale
...
C903  format(' 11',F6.1,F6.1,F6.2,F5.1,F5.2,F7.1,F5.2,F5.1,F5.0,2f7.1)  Original
C903  format(' 11',F6.1,F6.1,F6.2,F5.1,F5.2,F7.1,F5.2,F5.1,F5.0,2f11.5) Robin F.
 903  FORMAT(' 11',F6.1,F6.1,F6.2,F5.1,F5.2,F7.1,F5.2,F5.1,F5.0,2f8.3)
99      RETURN
C
        END
