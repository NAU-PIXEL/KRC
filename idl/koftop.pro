...
labg=['Fit toler  \ FIT_ABX','Frac.Scale / > Amoeba','----spare','----spare' $
...
,'                      |leave: 0=clear' ]
parg=[.01, .05, -7,-7, 20.,1000., 220.,0.8,1.,1.,1 ] ;
...
117: kons=[72,74,75,76,-1,73,-1] ; k(T) solids in partcond,
; append 70,72,735 to overplot retsults with constant properties for solids
...
138: begin & kons=[25,48,481,315,5,33,335,56] ; H2O k
parr[11:13]=[120.,280,10.]  & end; confine to solid H2O range plus margin

139: kons=[ 49,491,315,5,33,25,5,482,335,57] ; Use H2O Cp data points
...
18: help,kon,ttt,yin,rrr,ggg,dd4,cc4,pp4             ; Guides and help
...
kon=60 & goto,dokon             ; set bbb to proper size
end
...
31: begin & imat=pari[1]        ; Thermal conductivity of minerals
...
for imat=k1,k2 do begin         ; analytic expressions
    ...
    for i=0,n2-1 do begin 
        ...
        ccc=poly_fit (xxx,yin,3,yfit, yband,sigma,corrm)
        ar=MEAN_STD(yfit-yin,std=std) 
        print,std,ccc,refc,form='(f9.5,'' >'',4g14.7,'' <'',2x,a)' 
        kbb=kbb+1
        bbb[*,kbb]=yin
...
;        PAUSE,-1
        ...
        print,jf,std,ccc,refc,form='(i3,f9.5,'' >'',4g14.7,'' <'',2x,a)'
        nyy=n_elements(yfit)
...
377: begin ; Convert to use 73+ OVERRIDES 72
ztit='Un-cemented' 
...
41: begin & imat=parc[1]        ; Specific heat capacity of minerals
...
44: begin  ; Test many SPECHEAT options
    ...
    for k=0,np-1 do begin
        ...
        yin=SPECHEAT(ttt,imat,aaa[k],arg4,refc,tlim)
        ...
        print,std,ccc,refc,form='(f9.5,'' >'',4g14.7,'' <'',2x,a)' 
        sss[k]=refc
        if kkk[k] eq 3 then plot,ttt,arg4,xtit='K',ytit='k'
        bbb[*,k]=yin
        ii=where(ttt lt tlim[0] or ttt gt tlim[3],j) ; out of T range
...
48: yin=SPECHEAT(ttt,9,1,arg4,refc,tlim) ; H2O k into arg4

481: yin=arg4                   ;+ yin=arg4

482: yfit=EVMONO(ccc,xxx); Make points on uniform T

49: yin=SPECHEAT([tmin,tmax],9,7,arg4,refc,tlim) ; Cp H2O data points
...
72: begin                    ; Compute particulate k for 1 or set cement volumes
