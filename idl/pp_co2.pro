FUNCTION PP_CO2, tin,heat,cond,dens,cp, ptype=ptype,vis=vis,txt=txt $
,condg=condg, gvis=gvis,gcp=gcp
...
; ptype in_	Int:  Which pressure curve to use. 
...
;                     -1=pressure   -2= HEAT CAPACITY  -3=viscosity  
;                     else=gas thermal conductiivity
;           OR  Float, Pressure in Pa, and func. is density of the gas (kg/m^3)
; vis	out_	fltarr(n) Viscosity of the solid ???
; txt   out_    String: ID of pressure type
; condg	out_ 	fltarr(n,m) Thermal conductivity of the gas, m=4 versions
; gvis	out_	fltarr(n) Dynamic viscosity of the gas.  Pa s
; gcp   out_    fltarr(n) Specific heat at constant pressure of gas  J/ kg.K
; func.	out.	vapor pressure <pascal>  OR density of the gas
...
; 2010aug05 HK Add options for gas density, viscosity and specific heat
;_End

ntin=n_elements(tin)
; VAPOR  PRESSURE:  see test code below

if size(ptype,/type) ge 4 then begin ; output gas density
; ideal gas law is pV=nRT or rho=1.e-3 M P / RT  where M is Molecular Weight
; but, use CO2 emperical values at 1atm ( 101325 Pa) and 0C
    vp= 1.977 *(ptype/1.01325E5)*(273.15/tin) ; gas density, kg/m^3
endif else begin
...
; VISCOSITY   Dynamic: 1 Pascal-second = 10 poise
; vis=exp(1.43262  + 0.00440360*TIN) ;???  viscosity  Pa.s 
;Can not find source of above, this is far off from the gas
 vis=7.e9 ; Solid viscosity @ 180K , Yamashita97CO2.pdf p 1329.9a Penetration
; effective viscosity at 180K 0.1MPa 1.E14  Relaxation

; for the Gas: p 1330.5a  [ of what??]
;http://encyclopedia.airliquide.com/Encyclopedia.asp?GasID=26
; Viscosity (1.013 bar and 0 C ) : 0.0001372 Poise
; 1 Pa.s is 10 Poise , used for dynamic viscosity
; viscosity of gases ~ mean velocit, ~ sqrt(T) [Present, eq 3-24)
; Lange p 1675, extract from table..
; visc=[-78.4,-20.7,    0,   12.,20., 100.,162.4] ; Celsius
; visv=[103.3,129.4,138.3,143.6,160.,197.2,214.3] ; microPoise
; CRC F-43 
; crcc=[-97.8,-78.2, -60.,-40.2, -21.,-19.4,   0.,20.]  ; Celsius
; crcv=[ 89.6, 97.2,106.1,115.5,129.0,126.0,139.0,148.] ; micropoise

mu0=1.48E-5 ; Pa-s at 20C
sutc= 240. ; Sutherland constant
viscg=mu0*((293.15+sutc)/(tin+sutc))*(tin/293.15)^1.5 ; gas. Sutherland's formula
; the above is close to gvis below
;http://www.engineeringtoolbox.com/carbon-dioxide-d_1000.html has Saturated
;Liquid
; Vesovic90CO2.pdf  data cover 203:1497K, in the limit of zero density
; does not say in dynamic or kinemaitc, does not give units.
; but Table IV indicates  micro Pa s 
;abc=[.235156,.4226199, 0., -0.491266, 0.6280115, 2.387869e-2 $ Table 1
;,5.211155e-2,-0.5387661,4.350794, 5.347906e-2,0.6735941,-10.33404]
ai=[.235156, -0.491266,5.211155e-2, 5.347906e-2,-1.537102e-2]; zero P limit
tstar=tin/251.196 & lnts=alog(tstar) ; Eq 5 and 6
lecs=evmono(ai,lnts) ; log of effective cross section  Eq 4
gvis=1.00697e-6*sqrt(tin)/exp(lecs) ; Pa.s, Eq 3. Agrees with Append IV table

; SPECIFIC HEAT at constant pressure of gas
;http://www.engineeringtoolbox.com/carbon-dioxide-d_974.html  kj/kgK
gsh=[0.709,.735,.763,.791,.819,.846,.871,.895,.918,.939] ; nearly linear
gst=175.+25*findgen(n_elements(gsh))
gcp=fltarr(ntin)
for i=0,ntin-1 do begin 
    fx=RNDEX(tin[i],gst)
    gcp[i]=1.e3*RTERP(fx,gsh,/ext) ; convert to J/kg.K
endfor
;-------------------------------------------------------
