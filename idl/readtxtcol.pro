...
; fill	in_	If set, will fill missing columns with this value with leading 
;               blank. Else too few cols stops. 
;               Default is to skip completely blank lines
; quiet	in_	If set, will not report rows with too few columns
...
savetop=arg_present(top) and nskip gt 0 
if savetop then top=strarr(nskip)

nm1=long(maxrow-1)		; last allowed read
buf = 'dum'
if !dbug then help,nskip
if nskip gt 0 then for i=1,nskip do begin 
    readf, format='(A)',ilun,buf
    if savetop then top[i-1]=buf
endfor
kskip=0
if nskip lt 0 then repeat begin 
    readf,format='(A10)',ilun,buf 
    kskip=kskip+1
endrep until strpos(buf,'C_END') ge 0
if !dbug then print,buf
