...
C             5=ECLIPSE failed 7=TFINE failed
...
C
      ...
      INTEGER*4 I,II,IH,IP,J,J7,JJ,JJH,JJJ,JJP,JRSET,J3P1,K,KG,KN
     & ,KM,N1P1,NZ
C
...
C variables for k(T)
      ...
      INTEGER IK1,IK2,IK3,IK4,IKK(4)   ! layer indices for kofT
...
C     cumulative center-depth in units of local scale
 ...
 156  FORMAT (' Lower layer of time doubling: ',15I3)
      ...
      LRARE = (PARC(1).GE. 1.3) ! eclipse on last day
      IF (LRARE) THEN ! plan for  RARE eclipse
        JBE(1)=0  ! do not need detailed profile here
        PARI(1)=N2
        PARI(2)=IOERR
        CALL ECLIPSE (PARC,PARI, JBE, FINSOL)
        IF (FINSOL(1) .LT. 0. .OR. (JBE(1).GT.JBE(2))) THEN ! ERROR
          WRITE(IOERR,*)'TDAY(1: No ECLIPSE or returned error'
...
C used only with atmosphere. Ensure divisor CPOG is not zero
      ...
      J7=-1                     ! ensure  TFINE trigger is turned off
...
C
      DO 320 JJJ=1,N3
        ...
        IF (LDAY) THEN 
          ...
          IF (LRARE .AND. J5.EQ.N5) J7=JBE(1)-1 ! turn on trigger for  TFINE
        ENDIF
        IF (LRESET) THEN
          DO  J=2,N1
            TAVE(J)=0.
...
C Set the boundary conditions
          ...
          IF (JJ.EQ.J7) THEN ! at a  Rare eclipse trigger
            IF (J7 .LE. JBE(1)) THEN ! at start of  Rare  eclipse period
C  Last arg is number of good layers in TTF, must not change in time loop
              CALL TFINE8 (2, KTT,DENN,CTT,IKK, TTF,KG)
              J7=2*JBE(2)-JBE(1)+1 ! move trigger to end of eclipse+followon
              J7=MIN(J7,N2-1)   !  TFINE always leaves 1 step to midnight
            ELSE                ! at the end of eclipse period
              ...
              WRITE(IOSP,33)'End eclipse: JJ,KG,delT,delE',JJ,KG,QA,QB
 33           FORMAT(A,I5,I4,2G15.5)
              
 22           FORMAT(99F8.3)
              IF (IDB5.GE.4)WRITE(47,22)(TTJ(I),I=1,N1) ! coarse  T
              CALL MVD(TTF,TTJ,KG) ! transfer the layers
              J7=1   ! no more  Rare eclipse action  But continue fort 46 output
            ENDIF
          ENDIF
C     
...
C FEMIT=FAC6F*SIGSB*TFNOW**4 is [[skyfac]]*Femis*sig*Tf^4
     ...
     &               + FAC6F*ATMRAD + SHEATF - FEMIT
            IF (LPH) POWER=POWER+EMIS*PLANH(JJ)+(1.D0-QA)*PLANV(JJ) ! planetary
C If fff, add back-radiation=(1-skyfac)*femis*emis_x*sig*Tfar^4
 ...
 230        TS3=TSUR**3         ! bare ground
            ...
            IF (TSUR.LT.TFTEST) THEN ! Frost will form.  Never unless atm.
...
C NEXT 3 LINE DEBUG ONLY.  Only on last day of last season for Rare eclipse
          IF (J7.GT.0 .AND. IDB5.GE.7) WRITE(46,244) 
