PRO toothb, kp,jj, vv=vv,fmt=fmt,tab=tab
...
; tab   in_     Flag. If set, uses index for color, rather than !binc[index]
;_Usage
...
; 2010jun09 HK Add keyword tab. Option [14] for value to be same color as tooth
;_End
...
,' "  Character size' $
,' " color index, -=tooth color','[dbug]','top-n50 for value alignment' $
,'Fraction of range for first color' $
,'Flag, 1=print exp(value) -1=print alog(value)']
par0=[0.25,0.1,.0025,3.,2., 1.25,1.5,1.8,255,255 $
...
if not keyword_set(tab) then qq=!binc[qq]         ; use !binc.  else use color table
