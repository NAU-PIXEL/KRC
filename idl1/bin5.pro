...
;             =4 will print debug values and stop before return
...
i=STRPOS(types,arch)  & if i gt postyp0 then enhere=2 else enhere=1
...
if keyword_set(arc) then begin  ; change type
    ...
    if i ge 0 then begin        ; only if user request was valid
        if i gt postyp0 then enfile=2 else enfile=1
