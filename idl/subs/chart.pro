...
,clip=clip,fmt=fmt,evod=evod,xin=xin,kim=kim ,tloc=tloc $
...
xloc=xa+.03*(xb-xa)	; a small % from left edge
...
;    j=where (finite(yy),ngood)
  ...
  if c1 then XYOUTS,xloc,k+tloc[k],sout,charsize=csize,charthick=cthick ; description
endfor
