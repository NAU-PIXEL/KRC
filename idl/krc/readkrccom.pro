...
;             [3] is nwkrc == number of 4-byte words in  KRCCOM
...
; If done with file: q=READKRCCOM(-1,hold) or free_lun,hold[0]
;_Calls   DEFINEKRC
...
if n_elements(hold) lt 3 then lun=0 else lun=hold[0] ; Is lun assigned ?
...
lun=hold[0]                 ; current logical unit
if arg1 lt 0 then begin       ; close the current file and free the logical unit
    free_lun,lun
    hold[0]=0 
    return,0
endif else if arg1 eq 0 or arg1 gt hold[2] then begin 
