...
\author{Hugh H. Kieffer  \ \ File=-/krc/Doc/helplist.tex Version 3.6.1 2018oct30}
%_History
...
\qi Eclipses by the planet of satellite.
\qi Incident flux from a planet onto a satellite, sinusoidal.
...
touched; they could be replaced by the results of running PORBMN .

The parameter title lines are skipped, so that you may put comments there
carefully.
...
subroutine setsit; as the routine name in lowercase just below the parameter
name.
\vspace{-3.mm} 
\begin{verbatim} 
 - - - - - - - - - - - - - - - - - - - -
...
Index=3 will read .tmx file; if that has Tatm it will set the held fff
atmosphere only if prior index=4 is not active. Index=4 will set held fff
atmosphere; file must be .tm3 or KRC will quit.
