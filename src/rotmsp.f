...
C_PAUSE
	...
	SUBROUTINE ROTORB (ODE,CLIN,ARGP, A, LRAD)
...
C_Pause
	...
	FAC=RPD
	IF (LRAD) FAC=1.0	! already in radians
...
C-      q2=(VFDOT(VIN,XX)/VFDOT(XX,XX))*VFMAG(XX) ! full math
...
C then, 3 independant binary tests for sign of  Y axis
	...
	SUBROUTINE ROTEST (BB, DER,DEV)
C_Titl  ROTEST  Tests deviation of matrix from a rotation matrix
	IMPLICIT NONE
C_Arguments
	REAL*4 BB(9)		! IN. Array to be tested
	REAL*4 DER  ! out   Deviation of determinant of matrix from 1.
	REAL*4 DEV  ! out.  Max. deviation of mag.^2 of rows and columns from 1.
C Desc
...
C_End
	REAL*4 R1,R2,R3,C1,C2,C3,YA,YB
	REAL*4 ONE /1./		! which precision to use
	...
	YA=MIN(R1,R2,R3,C1,C2,C3) ! smallest
	YB=MAX(R1,R2,R3,C1,C2,C3) ! largest 
	
	R1=ONE-YA		! smallest relative to 1
	R2=YB-ONE		! largest relative to one
	DEV=MAX(R1,R2)		! maximum deviation from perfect
...
C compute determinant    comments are normal matrix notation
	...
	DER=ABS(R3-ONE)	! deviation from perfect
