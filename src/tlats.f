...
C	SKYFAC = 1.-SLOPE/180.  ! fraction of sky remaining
	SKYFAC = (1.+ COS(SLOPE/RAD))/2. ! effective isotropic radiation.
        JHOT=INT(FLOAT(N24)*13.5/24.) ! index of warmest time of day
	ZENLIM = FD(32)		! conical hole edge zenith angle, degrees
	COSZLIM=0.		! default limit is 90 degrees
	IF (ZENLIM.GT.0.) COSZLIM=COS(ZENLIM/RAD) !  
...
C Get atmosphere transmission and heating for horizontal surface
	  IF (COSI.GT.ACOSLIM) THEN ! Day: Sun is above horizon
	    CALL DEDING2 (OMEGA,G0,AVE_A,COSI,OPACITY, BOND,COLL,RI)
...
C Get diffuse insolation, including twilight and first-order surface reflection 
	  IF (COSI.GT.TWILIM) THEN ! in day or twilight zone
	    ...
	    CALL DEDING2 (OMEGA,G0,AVE_A,COS3,OPACITY, BOND,COL3,RI)
	    BOTDOWN=PI*(RI(1,2)+F23*RI(2,2)) ! diffuse flux down at surf.
	    DIFFUSE=SKYFAC*BOTDOWN ! diffuse flux onto surface
	    IF (ZENLIM.GT.0.) THEN ! bounce in a pit
	      AINC=ACOS(COSI)*RAD  ! incidence angle in degrees
	      G1=MIN(1.,(90.-AINC)/ZENLIM)
	    ELSE
	      G1=1.0
	    ENDIF 
