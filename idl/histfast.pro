...
; i1	out_	Lonarr(4), [nbin, #lo, #hi, nbad]    Used for the Histogram
...
if isint and (long(a2)-long(a1)) le 255 then begin
    bsize=1
...
yfac=1.-.06*(findgen(4)+1.)          ; normalized location of annotations
xloc=b1+0.04*bsize*(nbin-1)
j=(!p.multi[2]>1)<4             ; # plot rows per page
csize=[1., 1.,0.65,0.5,0.35]
csize=csize[j]
...
XYOUTS,xloc,yloc[0],'Mean & S.D.='+string(mean,sd),charsize=csize
XYOUTS,xloc,yloc[1],'Extremes='+string(a1,a2),charsize=csize
