PRO chart, yyy, title=title,parti=parti,xtit=xtit,range=range,dlin=dlin,psy=psy $
     ,clip=clip,fmt=fmt,evod=evod,xin=xin,csize=csize,cthick=cthick,oplot=oplot
...
; xin	in_	array(nd) of X-axis values. Default is count. 
...
for k=0,mp-1 do begin	; each panel
    ...
    if ngood eq 0 then sran='all NaN' else begin
      ...
      if ymax ne ymin then begin ; have changing data
          ...
          if do2 then begin     ; indicate odd and even points
              ...
              if evod[4] ge 0 then OPLOT,XAX,   y+ky,line=bolin,color=bocol
          endif else if psy eq 0 then OPLOT,XAX,y+ky,line=dine  $ ; data line
          else OPLOT,XAX,y+ky,psym=psy
      endif
  ...
  if c1 then XYOUTS,xloc,k+0.4,sout,charsize=csize,charthick=cthick ; description
endfor

;option for pstrip separators
if c1 and keyword_set(dlin) then for k=1,mp-1 do plots,[0.,xb],[k,k],line=dlin
