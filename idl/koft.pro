...
;             11:19 Interpolates data points and also returns them. 11:17 defined
...
;             kode 24, fltarr(3), 0]=tolerance for fit 1]=frac for 1st try
;                        2]=print flag
...
; 2011jul25 HK Use arg3[2] as print flag
...
ia3=fix(arg3[0])                ; material index for several kodes
...
8: begin & ref='IceIh_Hobbs74' ; Ice Ih
out=488.91/tin+0.4685 ;  <J m^-1 s^-1 K^-1> Equation 5.6, page 360
tlim=[108.,108.,273.15,273.2]
end
...
17: begin & ref='IceIh_Slack80'
ddd=[10,1.2,15,.60,20,.38,40,.161,80,.081,100,.065,120,.054,150,.043,200,.032,250,.024,273.2,.0214] ; T,K  W/cm-K
i=n_elements(ddd) 
nt=i/2                          ; number of data points
ddd=reform(ddd,2,nt)
ddx=reform(ddd[0,*])
ddy=100.*reform(ddd[1,*]) ; convert to W/m-K
tlim=[8.,10.,273.2,273.2]
end
...
if arg3[2] ne 0 then  print,'FIT_ABX out:',pfit,form='(a,2g12.5,f6.0,g12.7)'
...
if kode ge 11 and kode le 17 then begin ; interpolate data to request
