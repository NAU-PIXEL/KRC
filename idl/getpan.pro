...
; labs  in_	strarr labels (description) for individual items
...
if not keyword_set(labs) then kode=0 $ ; no labels or extensions
  else if n_elements(labs) eq n and size(labs,/type) eq 7 then kode=1 $ ; labels, fixed dimension
